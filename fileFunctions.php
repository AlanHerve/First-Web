...
function checkFile($file_name){
    ...
    if(isset($_POST["processForm"]) && $_FILES[$file_name]["tmp_name"]!=NULL) {
      $check = getimagesize($_FILES[$file_name]["tmp_name"]);
      if($check !== false) {
        //echo "File is an image - " . $check["mime"] . ".";
        $uploadOk = 1;
      } else {
        $error = "File is not an image. / IS EMPTY";
        $uploadOk = 0;
      }
      ...
      if (file_exists($target_file)) {
        $error = "<p>Sorry, file already exists.</p>";
        $already = 1;
      }
      ...
      if ($uploadOk == 0) {
        $error = "<p>Sorry, your file was not uploaded.</p>";
